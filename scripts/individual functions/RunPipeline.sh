...
run_trimmomatic (){
  input_dir="$1"
  output_dir="$2"
  r1_pattern="${3:-*_R1.001.fastq}"
  threads="${4:-8}"
  conda_env="${5:-trimmomatic_env}"
  adapters_path="${6:-/path/to/TruSeq3-PE-2.fa}"
  ...
  do
    # Obtener basename quitando el sufijo _R1.001.fastq (ajusta el sufijo si tu nomenclatura es distinta)
    BASENAME=$(basename "$R1" "_R1.001.fastq")
    ...
    trimmomatic PE -threads "$threads" -phred33 \
      ...
      ILLUMINACLIP:"$adapters_path":2:30:10:2:keepBothReads \
...
run_star_align (){
  ...
  genome_dir="$2"
  gtf_file="$3"
  outdir="${4:-/Users/JK/Desktop/Mapping_STAR/}"
  r1_pattern="${5:-*R1_001.fastq}"
  threads="${6:-8}"
  conda_env="${7:-mapping}"
  ...
  for R1 in "$input_dir"/$r1_pattern; do
    R2="${R1/R1_001/R2_001}"
    sample_names=$(basename "$R1" "R1_001.fastq")
